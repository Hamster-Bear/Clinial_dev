...
- **管理员页公共壳**: `admin_manager.R` 已接入 `modules/common/ui_shell.R` 公共卡片壳，统一系统入口卡、摘要卡、说明块与主要管理卡片视觉；管理员页当前继续保持“摘要优先、明细随后”的信息结构和账号总览联动逻辑，但已将负责人迁移与协作授权合并为单一“数据空间管理”卡片，并将系统概览、异常态势摘要与账号状态管理统一切到更紧凑的统计卡布局。
