...
- **前端展示文案边界**: `subtitle`、`app_card_note`、`helpText`、`help_text`、`note` 等用户可见文案只描述当前能力、操作边界和结果含义；不得暴露“本轮只统一”“继续保留”“当前统一”“继续沿用”“统一收纳”“暂不额外暴露”“不改绘图/导出”以及“开发阶段”“内部使用”“暂定”“后续可”“暂时”等开发阶段口径，也避免使用“总入口内”“公共复用”“动态 UI”“类型分支”“代码草稿”“不改变已有计算和生成逻辑”“保持原有 server 校验”“统一空状态提示”“保持原有处理方式”“统一认证入口”“继续围绕邮箱身份扩展”“后续新增账号相关入口”“聚合展示”“协作工作台”“不改变原有加载能力”“不改动数据库侧原有结构”“系统管理入口”“集中处理”“统一筛查”“统一排查与筛查”“当前卡片统一处理”“数据空间工作台”“统一完成整理与导入”“阶段二将”“集中到同一张功能卡片”“改为登录后自助完成”“仅保留基础密码修改”“不扩展其它账号管理能力”“开放前”等内部实现、结构视角或实现保持型表述。
- **源码注释边界**: `#` inline 注释只说明当前约束、数据语义、兼容性原因或调用前提；不记录“重构版”“暂时保留”“后续可”“优化”这类开发过程、实现阶段或重构历史说明。
...
- **管理员页公共壳**: `admin_manager.R` 已接入 `modules/common/ui_shell.R` 公共卡片壳，统一系统入口卡、摘要卡、说明块与主要管理卡片视觉；管理员页当前继续保持“摘要优先、明细随后”的信息结构和账号总览联动逻辑，但已将负责人迁移与协作授权合并为单一“数据空间管理”卡片，并将系统概览、异常态势摘要与账号状态管理统一切到更紧凑的统计卡布局。统一空间选择器 `selected_manage_workspace_id()` 在回退到默认空间前，必须先处理零长度 `workspace_ids`，空向量时返回空字符串而不是直接取首项。
- **应用首屏加载反馈**: `app.R` 必须通过 `modules/common/ui_shell.R` 的公共 helper 提供全局 loading overlay，避免浏览器在模块 source、数据库初始化和初次渲染期间长时间显示空白；该 overlay 当前采用 `www/assets/loading/hamster.svg` 作为 loading 主视觉，并通过 `www/assets/loading/loading.css` 承载覆盖层与轨道流动动效，同时支持“正在连接服务 / 正在初始化模块 / 正在进入工作台”等阶段文案，在首次 `shiny:idle` 后自动隐藏，并继续复用到登录/注册等认证链路的 loading 反馈。
- **登录后加载文案时序**: 登录成功后若显示“正在进入工作台”，不得在同一服务分支通过立即 `hide` 立刻关闭 overlay；应采用显式成功/失败分支控制，成功后允许短暂延迟隐藏，确保用户可感知到状态切换。
