...
auth_with_transaction <- function(db, code) {
  transactional_code <- eval(substitute(function(conn) code), envir = parent.frame())

  if (inherits(db, "Pool")) {
    return(pool::poolWithTransaction(db, transactional_code))
  }
  ...
  result <- transactional_code(conn)
  DBI::dbCommit(conn)
  committed <- TRUE
  result
}
