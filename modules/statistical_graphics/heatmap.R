  ...
  tagList(
    fluidRow(
      box(
        ...
        fluidRow(
          column(8,
                 selectizeInput(ns("heatmap_vars"), "选择数值变量", 
                              choices = NULL, multiple = TRUE)
          ),
          column(4,
                 checkboxInput(ns("heatmap_cluster"), "显示聚类", value = TRUE)
          )
        ),
...
heatmap_server <- function(input, output, session, data) {
  ...
  observeEvent(input$render_plot, {
    ...
    tryCatch({
      p <- create_heatmap()
      final_plot(p)
      showNotification("热图生成完成", type = "message")
    }, error = function(e) {
      showNotification(paste("热图生成错误:", e$message), type = "error")
      final_plot(NULL)
    })
  })
