...
expect_contains(module_text, "user_profile_ui <- function\\(id\\) \\{[\\s\\S]*?copy <- ACCOUNT_ENTRY_COPY\\$profile", "用户信息模块 UI 作用域初始化共享文案")
expect_contains(module_text, "copy <- ACCOUNT_ENTRY_COPY\\$profile", "用户信息模块复用共享文案")
expect_contains(module_text, "title = copy\\$title", "用户信息模块标题")
expect_contains(module_text, "subtitle = copy\\$subtitle", "用户信息模块副标题")
expect_contains(module_text, "uiOutput\\(ns\\(\"profile_content\"\\)\\)", "用户信息模块主体输出")
expect_contains(module_text, "build_profile_notice <- function", "用户信息模块说明文案构建函数")
expect_contains(module_text, "build_profile_content", "用户信息模块独立构建函数")
expect_contains(module_text, "app_card_note\\(build_profile_notice\\(user\\)\\)", "用户信息模块在首屏主卡展示说明")
expect_contains(module_text, "title = copy\\$workbench_title", "用户信息模块安全工作台")
expect_contains(module_text, "app_stat_card\\(", "用户信息模块概览统计卡")
expect_contains(module_text, "tabBox\\(", "用户信息模块功能聚合标签页")
expect_contains(module_text, "copy\\$tabs\\$verify_email", "用户信息模块邮箱验证标签页")
expect_contains(module_text, "class = \"app-action-row\"", "用户信息模块使用紧凑动作区")
expect_contains(module_text, "class = \"btn-info app-action-btn\"", "用户信息模块发送按钮使用公共动作按钮样式")
expect_contains(module_text, "class = \"btn-primary app-action-btn\"", "用户信息模块确认按钮使用公共动作按钮样式")
...
expect_not_contains(module_text, "width = \"100%\"", "用户信息模块按钮不应默认满宽")
