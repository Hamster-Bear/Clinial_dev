...
expect_contains(module_text, "permission_manager_ui <- function\\(id\\) \\{[\\s\\S]*?copy <- ACCOUNT_ENTRY_COPY\\$permissions", "权限管理模块 UI 作用域初始化共享文案")
expect_contains(module_text, "copy <- ACCOUNT_ENTRY_COPY\\$permissions", "权限管理模块复用共享文案")
expect_contains(module_text, "title = copy\\$title", "权限管理模块标题")
expect_contains(module_text, "subtitle = copy\\$subtitle", "权限管理模块副标题")
...
expect_contains(module_text, "title = copy\\$accessible_title", "权限管理模块普通成员信息卡")
expect_contains(module_text, "title = copy\\$workbench_title", "权限管理模块协作工作台")
expect_contains(module_text, "app_stat_card\\(", "权限管理模块概览统计卡")
expect_contains(module_text, "class = \"app-action-row\"", "权限管理模块使用紧凑动作区")
expect_contains(module_text, "class = \"btn-primary app-action-btn\"", "权限管理模块授权按钮使用公共动作按钮样式")
expect_contains(module_text, "class = \"btn-danger app-action-btn\"", "权限管理模块撤销按钮使用公共动作按钮样式")
...
expect_contains(module_text, "selectInput\\(session\\$ns\\(\"managed_workspace_id_transfer\"\\), \"选择要迁移负责人的数据空间\"", "权限管理模块负责人迁移空间选择器")
expect_contains(module_text, "tabBox\\(", "权限管理模块预览使用标签页")
expect_contains(module_text, "permission_workbench_tabs", "权限管理模块聚合工作台标签页")
...
expect_not_contains(module_text, "width = \"100%\"", "权限管理模块按钮不应默认满宽")
