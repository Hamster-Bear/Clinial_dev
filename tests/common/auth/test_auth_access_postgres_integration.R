...
with_isolated_auth_pool <- function(schema_name, code) {
  skip_if_no_test_db()
  if (!requireNamespace("pool", quietly = TRUE)) {
    skip("缺少 pool，跳过 pool 集成测试")
  }
  ...
  pg_pool <- tryCatch(
    ...
    error = function(e) {
      skip(paste0("PostgreSQL pool 初始化失败：", e$message))
    }
  )
  on.exit(try(pool::poolClose(pg_pool), silent = TRUE), add = TRUE)
...
test_that("pool 模式下注册不会因事务 API 报假失败", {
  with_isolated_auth_schema(function(conn, schema_name) {
    expect_true(nzchar(schema_name))

    with_isolated_auth_pool(schema_name, function(pg_pool) {
      with_env_vars(
        c(
        ...
        {
          created <- auth_register_user(pg_pool, "pool_user", "pool_user@test.local", "StrongPass123")
          expect_true(isTRUE(created$success))
          expect_match(created$message, "注册成功，请登录")

          created_row <- auth_get_user_by_email(pg_pool, "pool_user@test.local")
          expect_equal(nrow(created_row), 1)

          login_result <- auth_authenticate_user(pg_pool, "pool_user@test.local", "StrongPass123")
          expect_true(isTRUE(login_result$success))
        }
      )
    })
  })
...
test_that("pool 模式下用户管理事务链路保持一致", {
  with_isolated_auth_schema(function(conn, schema_name) {
    expect_true(nzchar(schema_name))
    ...
    with_isolated_auth_pool(schema_name, function(pg_pool) {
      with_env_vars(
        ...
        {
          owner <- auth_register_user(pg_pool, "pool_owner", "pool_owner@test.local", "StrongPass123")
          target <- auth_register_user(pg_pool, "pool_target", "pool_target@test.local", "StrongPass123")
          expect_true(isTRUE(owner$success))
          expect_true(isTRUE(target$success))

          acting_user <- list(id = owner$user$id, is_admin = FALSE)
          workspace <- service_create_workspace(pg_pool, "pool-service-space", owner$user$id)
          expect_true(nzchar(workspace$id))

          grant_result <- service_grant_workspace_access_by_email(
            pg_pool,
            workspace$id,
            target$user$email,
            "editor",
          ...
          expect_identical(grant_result$mode, "membership")

          memberships <- service_list_workspace_memberships(pg_pool, workspace$id)
          target_membership <- memberships[memberships$user_id == target$user$id, , drop = FALSE]
          expect_equal(nrow(target_membership), 1)
          expect_identical(target_membership$role[[1]], "editor")

          transfer_result <- service_transfer_workspace_owner_by_email(
            pg_pool,
            workspace$id,
            target$user$email,
            acting_user = acting_user
          )
          expect_identical(transfer_result$mode, "owner")

          workspace_after <- service_get_workspace(pg_pool, workspace$id)
          expect_equal(workspace_after$owner_user_id[[1]], target$user$id)

          memberships_after <- service_list_workspace_memberships(pg_pool, workspace$id)
          owner_membership <- memberships_after[memberships_after$user_id == owner$user$id, , drop = FALSE]
          target_membership_after <- memberships_after[memberships_after$user_id == target$user$id, , drop = FALSE]
          expect_equal(nrow(owner_membership), 1)
          expect_equal(nrow(target_membership_after), 1)
          expect_identical(owner_membership$role[[1]], "editor")
          expect_identical(target_membership_after$role[[1]], "owner")
        }
      )
    })
  })
