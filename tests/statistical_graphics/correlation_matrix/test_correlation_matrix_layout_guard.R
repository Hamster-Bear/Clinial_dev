...
source_text <- paste(readLines(file.path(project_root, "modules", "statistical_graphics", "correlation_matrix.R"),
  encoding = "UTF-8", warn = FALSE), collapse = "\n")

test_that("相关性矩阵模块使用统一三卡外层结构", {
...
test_that("相关性矩阵模块包含结果区动作条和结果页签", {
...
test_that("相关性矩阵模块不包含已废弃的裸 box 或 wellPanel 包装", {
...
test_that("相关性矩阵模块包含 task_history state/apply_state 契约", {
