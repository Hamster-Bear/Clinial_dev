...
test_that("run_auth_regression.ps1 会校验环境并串联关键测试", {
  ...
  expect_match(txt, 'tests/common/auth/test_email_service_helpers.R', fixed = TRUE)
  expect_match(txt, 'tests/common/auth/test_auth_helpers.R', fixed = TRUE)
  expect_match(txt, 'tests/common/auth/test_account_service_helpers.R', fixed = TRUE)
  expect_match(txt, 'tests/workspace_access_manager/test_workspace_access_manager_guard.R', fixed = TRUE)
  expect_match(txt, 'tests/root/test_access_boundary_guard.R', fixed = TRUE)
  expect_match(txt, 'tests/root/test_project_docs_guard.R', fixed = TRUE)
  expect_match(txt, 'tests/common/auth/test_auth_access_postgres_integration.R', fixed = TRUE)
