...
- 管理员操作入口位于系统管理页；workspace、membership、invite 与 owner 迁移能力已统一下沉到 service 层。
...
- 数据库管理页使用公共卡片壳：统一主卡片、说明块、锁定提示、上下文摘要与结构总览摘要卡，但保持原有“空间与目录 / 上传与导入 / 结构总览”页签结构不变。
- 统计分析总入口使用公共卡片壳：统一全局筛选卡、方法选择卡、参数设置卡、结果卡与导出说明面板，但保持原有“统计表格 / 统计报告 / 可复现代码”结果结构以及各统计子模块参数 UI、分析逻辑不变。
