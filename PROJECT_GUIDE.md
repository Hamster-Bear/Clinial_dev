...
1. **参考亚组灵活配置**: 目前亚组分析默认第一水平为参考组，未来需在 UI 暴露“参考组选择”下拉框 （回归分析部分已实现）。
...
1. **解耦“数据提取”与“视图渲染” (CTO 建议)**:
   目前的 `build_unified_regression_table` 仍是典型的过程式巨型函数。未来应拆分为 `compute_regression_stats()`（纯数据计算引擎，输出 JSON/DataFrame 元数据）和 `render_clinical_table()`（纯视图渲染引擎）。这不仅能极大提升代码可读性，也为后续平滑迁移至 React 前端 + Python 微服务架构铺平道路。
2. **声明式表头映射 (CTO 建议)**:
   借鉴 `tfrmt` 包的理念，在展开列分组 (Pivot wider) 时，引入声明式的模板映射字典。使用户可通过修改 JSON 配置文件来定制表头结构，而非硬编码拼接列名。
3. **防御性建模设计 (Defensive Modeling)**:
   在将数据推入 `glm` 或 `coxph` 前，增加轻量级的数据检查阀门（如检查每个因子层级的事件数是否大于阈值）。若数据极端稀疏，则直接短路返回“样本量不足”的状态码，避免消耗服务器资源并抛出底层迭代失败的 Warning。
4. **可复现代码沙盒**: 为自动生成的 R 代码片段增加“内置最小示例数据集”开关，方便用户一键复制并在本地 RStudio 成功运行。
